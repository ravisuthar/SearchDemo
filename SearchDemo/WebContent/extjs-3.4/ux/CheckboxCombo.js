...
/**
    ...
    initComponent: function() {
        ...
        this.addEvents('expand', 'collapse', 'change', 'checkedSelectAll');
...
/*this.cbgroup = new Ext.form.CheckboxGroup({
    ...
    onBeforeLoad: function() {
        ...
        var cbgroup = this.cbgroup;
        if (typeof cbgroup != 'undefined' && cbgroup != null) {
            this.checkboxValues = cbgroup.getValue();
        }
        this.removeCheckboxes();
    },
    // private
    ...
    addCheckboxes: function() {
        ...
        Ext.each(this.store.data.items, function(rec) {
            if (rec.data[this.displayField] == this.selectAllLabel) {
                checkboxes.push({
                    name: rec.id,
                    boxLabel: this.tpl.apply(rec.data),
                    inputValue: rec.data[this.valueField],
                    checked: (rec.checked ? rec.checked : false),
                    listeners: {
                        check: function(t, checked) {
                            me.fireEvent('checkedSelectAll', me, checked);
                        }
                    }
                });
            } else {
        ...
        var loadMask = this.innerList.child('.loading-indicator');
        if (typeof loadMask != 'undefined' && loadMask != null) {
            loadMask.remove();
        }
        if (this.store.getCount() == 0) {
            this.cbgroup = new Ext.form.CheckboxGroup({
