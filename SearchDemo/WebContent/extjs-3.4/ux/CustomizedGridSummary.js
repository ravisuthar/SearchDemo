...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
    init: function(grid) {
        ...
        this.rowTpl.compile();
        if (!this.cellTpl) {
            this.cellTpl = new Ext.Template('<td class="x-grid3-col x-grid3-cell x-grid3-td-{id} {css}" style="{style}">', '<div class="x-grid3-cell-inner x-grid3-col-{id}" unselectable="on" {attr}>{value}</div>', "</td>");
    ...
    calculate: function(rs, cm) {
        ...
        for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
            ...
            data[cname] = 0;
            if (cf.grandSummaryType) {
                for (var j = 0, jlen = rs.length; j < jlen; j++) {
                    var r = rs[j]; // get a single Record
                    data[cname] = Ext.ux.grid.GridSummary.Calculations[cf.grandSummaryType](r.get(cname), r, cname, data, j);
                }
            }
        }
        return data;
    ...
    renderSummary: function(o, cs, cm) {
        ...
        for (var i = 0, len = cs.length; i < len; i++) {
            ...
            if (cf.grandSummaryType || cf.grandSummaryRenderer) {
                p.value = (cf.grandSummaryRenderer || c.renderer)(o.data[c.name], p, o);
            } else {
                p.value = '';
            }
    ...
    refreshSummary: function() {
        var g = this.grid,
            ...
            cm = this.cm,
            rs = (ds.allData != undefined) ? ds.allData.items : ds.getRange(),
 ...
 * data - the cumulative data for the current column + grandSummaryType up to the current Record
